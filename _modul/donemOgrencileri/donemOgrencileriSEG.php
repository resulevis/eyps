...
if ( $islem == "ekle" ){
	...
	array_unshift($ogrenci_nolari, $_SESSION['program_id']);
	...
	FROM 
		tb_ogrenciler AS o
	LEFT JOIN 
		tb_donem_ogrencileri AS dog ON o.id = dog.ogrenci_id
	WHERE 
		o.program_id = ? AND
		o.ogrenci_no IN ( $in ) AND
		dog.ders_yili_donem_id IS NULL";
}
...
switch( $islem ) {
	case 'ekle':
		...
		foreach ($ogrenciler as $ogrenci ) {
			$sonuc = $vt->insert( $SQL_donem_ogrenci_ekle, array( $_SESSION[ "donem_id" ], $ogrenci[ "id" ] ) );
	...
	case 'sil':
		...
		$tek_ogrenci_oku = $vt->select( $SQL_tek_ogrenci_id_oku, array( $_SESSION[ "donem_id" ], $ogrenci_id ) ) [ 2 ];
		print_r($tek_ogrenci_oku);
		if (count( $tek_ogrenci_oku ) > 0) {
			$sonuc = $vt->delete( $SQL_sil, array( $_SESSION[ "donem_id" ], $ogrenci_id ) );
...
header( "Location:../../index.php?modul=donemOgrencileri");
?>